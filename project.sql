...
SELECT Gender, 
       ...
       AVG(DATEDIFF(CURDATE(), Birthday) / 365) AS Average_Age
FROM Customers
GROUP BY Gender;
...
-- Sales Analysis: Impact of Currency on Sales
-- This query examines sales by different currencies and converts them based on exchange rates.
SELECT o.CurrencyCode, 
       SUM(o.Quantity * p.Price * e.ExchangeRate) AS Total_Sales_Converted
FROM Orders o
JOIN Products p ON o.ProductKey = p.ProductKey
JOIN ExchangeRates e ON o.CurrencyCode = e.CurrencyCode
...
SELECT p.ProductKey, p.ProductName, 
       SUM(o.Quantity) AS Total_Quantity_Sold, 
       SUM(o.Quantity * p.Price) AS Total_Revenue, 
       SUM(o.Quantity * (p.Price - p.UnitCost)) AS Total_Profit
FROM Orders o
JOIN Products p ON o.ProductKey = p.ProductKey
GROUP BY p.ProductKey, p.ProductName
...
SELECT s.City, s.State, s.Country, s.Continent, 
       SUM(o.Quantity) AS Total_Quantity_Sold, 
       SUM(o.Quantity * p.Price) AS Total_Revenue
FROM Orders o
JOIN Products p ON o.ProductKey = p.ProductKey
JOIN Stores s ON o.StoreKey = s.StoreKey
GROUP BY s.City, s.State, s.Country, s.Continent
ORDER BY Total_Revenue DESC;
